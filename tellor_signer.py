...
def signValue(data):
    ...
    intData = int(data, 16)
    signatures = sign_cli(intKey, intData).split(' ')
    submitData["signatureR"] = str(signatures[0])
    submitData["signatureS"] = str(signatures[1])
...
def TellorSignerMain():
    ...
    submitData["starkKey"] = str(public_cli(
        int(EthKeyToStarkKey(privateKey), 16)))
    ...
    while True:
        ...
        for i in range(len(assets)):
            ...
            signValue(data)
            if assets[i]["timestamp"] - assets[i]["timeLastPushed"] > 300 or abs(
                    assets[i]["price"] - assets[i]["lastPushedPrice"]) > .05:
...
def testSubmit():
    data = {
        'starkKey': '0x13ebb76f3d0c31448a84bcfca6edc246637f3f6d8aa5ab2cb7e030d5b7c9034',
        'timestamp': 1607520077,
        'price': '561.77',
        'assetName': 'ETHUSD',
        'oracleName': 'Tellor',
        'signatureR': '0x46b6c32b6425b78661e0b017b290dd21893b0df1bbfd5045cc19e878a7ba832',
